...
class ProductListingService {
  Future<ApiResponse<ProductListingModel>> getListing(
      BuildContext context,String catId, String subId) async {
    var productProvider = Provider.of<ProductProvider>(context, listen: false);
    try {
      productProvider.setLoader = true;

      var response = await Api.getRequestData("products?cat_id=$catId&sub_cat_id=$subId", context);

      print(response.toString());

      ProductListingModel responseModel = ProductListingModel.fromJson(response);

      productProvider.setProductListingModel = responseModel;
